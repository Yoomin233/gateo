...
import { TickerDetailedInfo } from './prices.ts';
...
const PendingOrder = React.memo(
  (prop: Props & { token: TickerDetailedInfo }) => {
    const { order, scroll, token } = prop;
    ...
    const current_price = token.price;
...
const OrderWrapper = (prop: Props) => {
  const { order, scroll } = prop;
  const { balance } = React.useContext(AppContext);
  const token = get_ticker(order.market).toUpperCase();
  return (
    <PendingOrder
      order={order}
      scroll={scroll}
